"""Part 6: normal-impulse compensation continued from 44 deg down to 0 deg.

Same as part 5 but the release point starts from rest and receives an endpoint
normal impulse ``qd_plus = M(q)^-1 J(q)^T n j_n``; the script solves
``P_gamma(d_target; j_n) - d_target = 0`` on two impulse branches.

Importable interface::

    from run_normal_impulse_gamma_continuation import run
...
PART = "normal_impulse_gamma_continuation"
ALGO_VERSION = "v1"
...
DEFAULTS: dict[str, Any] = {
    ...
    "t_max": 8.0,
    "max_abs_impulse": 0.5,
    "initial_bracket_width": 0.005,
    "max_bracket_width": 0.2,
...
def qdot_after_endpoint_normal_impulse(q, impulse_n, model, normal):
    jacobian = model.free_endpoint_jacobian(q)
    generalized_impulse = jacobian.T @ (float(impulse_n) * normal)
    try:
        return np.linalg.solve(model.mass_matrix(q), generalized_impulse)
    except np.linalg.LinAlgError:
        return np.linalg.lstsq(model.mass_matrix(q), generalized_impulse, rcond=None)[0]
...
class GammaProblem:
    ...
    def release_state(self, d: float, impulse_n: float) -> BrachiationState:
        ...
        normal = slope_away_normal(self.slope)
        qd = qdot_after_endpoint_normal_impulse(q, impulse_n, self.model, normal)
        return BrachiationState(q=q, qd=qd, support_index=0)

    def induced_normal_velocity(self, d: float, impulse_n: float) -> float:
        state = self.release_state(d, impulse_n)
        normal = slope_away_normal(self.slope)
        endpoint_velocity = self.model.free_endpoint_jacobian(state.q) @ state.qd
        return float(normal @ endpoint_velocity)

    def evaluate_stride(self, d: float, impulse_n: float, stop_after_releases: int | None = 1) -> tuple[float, list]:
        samples = simulate(
            model=self.model, slope=self.slope, initial_state=self.release_state(d, impulse_n),
    ...
    def residual(self, impulse_n: float) -> float:
        d_next, _ = self.evaluate_stride(self.d_target, impulse_n)
        return d_next - self.d_target

    def stride_map_for_fixed_impulse(self, impulse_n: float) -> Callable[[np.ndarray], np.ndarray]:
        def p_of_d(x: np.ndarray) -> np.ndarray:
            d = float(np.asarray(x, dtype=float).reshape(-1)[0])
            d_next, _ = self.evaluate_stride(d, impulse_n)
            return np.array([d_next], dtype=float)
        return p_of_d

    def evaluate_root(self, branch_name: str, impulse_n: float) -> ContinuationRow:
        d_next, samples = self.evaluate_stride(self.d_target, impulse_n)
        state = self.release_state(self.d_target, impulse_n)
        energy = float(self.model.kinetic_energy(state.q, state.qd))
        induced_v_n = self.induced_normal_velocity(self.d_target, impulse_n)
        legality = evaluate_elbow_below_slope_section(samples, self.slope)
        try:
            _jac, _eig, rho = poincare_jacobian_eigenvalues_1d(self.stride_map_for_fixed_impulse(impulse_n), self.d_target, delta=1e-5, feasibility_check=self.feasibility)
            spectral_radius: float | None = float(rho)
        except Exception:
            spectral_radius = None
        stable = spectral_radius is not None and spectral_radius < 1.0
        return ContinuationRow(
            branch_name=branch_name, gamma_deg=self.gamma_deg, impulse_n=float(impulse_n), induced_v_n=float(induced_v_n),
            d_target=self.d_target, d_next=float(d_next), residual=float(d_next - self.d_target), spectral_radius=spectral_radius,
...
def find_impulse_roots(problem, bounds, points):
    grid = np.linspace(bounds[0], bounds[1], points)
    values = np.full(grid.shape, np.nan, dtype=float)
    for index, value in enumerate(grid):
        try:
...
def continue_branch(branch_name, seed_impulse, gamma_values, params, branch, cfg):
    rows: list[ContinuationRow] = []
    previous = float(seed_impulse)
    bounds = (-cfg["max_abs_impulse"], cfg["max_abs_impulse"])
    for gamma_deg in gamma_values:
        problem = GammaProblem(float(gamma_deg), params, cfg["d_target"], branch, cfg["dt"], cfg["t_max"])
        bracket = find_local_bracket(problem.residual, previous, cfg["initial_bracket_width"], cfg["max_bracket_width"], bounds)
        if bracket is None:
            bracket = find_scan_bracket(problem.residual, previous, bounds, cfg["fallback_scan_points"])
        if bracket is None:
            rows.append(ContinuationRow(branch_name, float(gamma_deg), float(previous), np.nan, cfg["d_target"], np.nan, np.nan, None, False, False, np.nan, None, "failed", "Could not bracket a root."))
            break
        try:
            root = brentq(problem.residual, bracket[0], bracket[1], xtol=cfg["xtol"], rtol=cfg["rtol"], maxiter=cfg["maxiter"])
            row = problem.evaluate_root(branch_name, root)
        except Exception as exc:
            rows.append(ContinuationRow(branch_name, float(gamma_deg), float(previous), np.nan, cfg["d_target"], np.nan, np.nan, None, False, False, np.nan, None, "failed", str(exc)))
            break
        rows.append(row)
        previous = row.impulse_n
    return rows
...
def _rows_to_arrays(rows: list[ContinuationRow]) -> dict[str, np.ndarray]:
    return {
        ...
        "impulse_n": np.array([r.impulse_n for r in rows], dtype=float),
        "induced_v_n": np.array([r.induced_v_n for r in rows], dtype=float),
...
def _compute(cfg: dict[str, Any]) -> tuple[dict[str, np.ndarray], dict[str, Any]]:
    ...
    seed_roots = find_impulse_roots(seed_problem, (-cfg["max_abs_impulse"], cfg["max_abs_impulse"]), cfg["fallback_scan_points"])
    if len(seed_roots) < 2:
        raise ValueError(f"Expected at least two impulse roots at gamma={cfg['gamma_start']}, found {len(seed_roots)}: {seed_roots}")
    ...
    low_seed = seed_rows[0].impulse_n
    high_seed = seed_rows[1].impulse_n
...
def _plot(arrays: dict[str, np.ndarray], summary: dict[str, Any], cfg: dict[str, Any]) -> dict[str, Figure]:
    ...
    for branch_name in sorted(set(arrays["branch_name"][ok])):
        ...
        gamma = gamma[order]
        axes[0, 0].plot(gamma, arrays["impulse_n"][sel][order], marker="o", label=branch_name)
        ...
        axes[1, 1].plot(gamma, arrays["induced_v_n"][sel][order], marker="o", label=branch_name)

    axes[0, 0].set_ylabel("normal impulse j_n [N s]")
    axes[0, 1].set_ylabel("post-impulse kinetic energy [J]")
    axes[1, 0].set_ylabel("spectral radius")
    axes[1, 1].set_ylabel("induced normal speed [m/s]")
    for ax in axes[1, :]:
    ...
    for ax in axes.ravel():
        ...
        ax.invert_xaxis()
        ax.legend()
    fig.suptitle(f"Normal-impulse compensation, target d={summary['d_target']:.4f} m, COM offset {summary['com_offset']:+.4f}")
    fig.tight_layout()
    return {"main": fig}
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Continue the two normal-impulse roots from gamma_start down to gamma_end.")
