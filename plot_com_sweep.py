"""Plot a cached COM-sweep result without rerunning the continuation."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Plot cached COM continuation sweep results.")
    parser.add_argument("result", nargs="?", type=Path, help="Path to a com_sweep_*.npz file.")
...
def main() -> int:
    ...
    x = data["com_offset"].astype(float)
    ...
    for value in switch_x:
        ...
        ax.text(value, y_max + 0.02 * y_span, f"x={value:+.3f}", rotation=90, ha="center", va="bottom", fontsize=8)
    ...
    ax.set_title(f"Cached adaptive COM continuation: branch={summary['branch']}, period={summary['period']}")
    ...
    output = args.output or result_path.with_name("com_sweep_latest.png")
    fig.savefig(output, dpi=180)
    print(f"Loaded: {result_path}")
    print(f"Wrote: {output}")
    print(f"Most stable COM offset: x={best_x:+.6f}, |lambda|max={best_lambda:.6f}")
    if switch_x:
        print("State-switch vertical lines at x =", ", ".join(f"{value:+.6f}" for value in switch_x))
    if args.show:
        plt.show()
    return 0
