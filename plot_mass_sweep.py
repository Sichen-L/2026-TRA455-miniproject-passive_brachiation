"""Plot a cached mass-sweep result without rerunning the continuation."""
...
def _latest_result(results_dir: Path) -> Path:
    candidates = sorted(results_dir.glob("mass_sweep_*.npz"), key=lambda path: path.stat().st_mtime)
    if not candidates:
        raise FileNotFoundError(f"No mass_sweep_*.npz files found in {results_dir}.")
    return candidates[-1]


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Plot cached mass-sweep results.")
    parser.add_argument("result", nargs="?", type=Path, help="Path to a mass_sweep_*.npz file.")
...
def main() -> int:
    ...
    x = data["mass_value"].astype(float)
    ...
    for value in switch_x:
        ...
        ax.text(value, y_max + 0.02 * y_span, f"m={value:.3f}", rotation=90, ha="center", va="bottom", fontsize=8)
    ...
    best_mass = float(summary["best_lambda_mass"])
    best_lambda = float(summary["best_lambda"])
    best_idx = int(np.argmin(np.abs(x - best_mass)))
    ax.axvline(best_mass, color="tab:green", linewidth=1.2, label="minimum |lambda|max")
    ax.annotate(
    ...
    output = args.output or result_path.with_name("mass_sweep_latest.png")
    fig.savefig(output, dpi=180)
    print(f"Loaded: {result_path}")
    print(f"Wrote: {output}")
    print(f"Most stable mass: {summary['mass_target']}={best_mass:.6f}, |lambda|max={best_lambda:.6f}")
    if switch_x:
        print("State-switch vertical lines at mass =", ", ".join(f"{value:.6f}" for value in switch_x))
    if args.show:
        plt.show()
    return 0
